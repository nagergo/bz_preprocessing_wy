function InstantaneousTheta = getTheta_wy(basepath,varargin)
% Calculates the instantaneous theta with phase and power
% By Peter Petersenl,Last edited: 18-08-2019
% edited by Winnie Yang, Jan 2022
p = inputParser;
...
cd(basepath);
load([basename,'.session']);
saveAsFullfile = fullfile(basepath,[session.general.name,'.',saveAs,'.lfp.mat']);
%%
...
if ~exist(saveAsFullfile,'file') || forceReload
    if ~exist(fullfile(basepath,[session.general.name, '.lfp']),'file')
...
%         bz_LFPfromDat(session.general.basepath)
    ...
    signal = session.extracellular.leastSignificantBit * double(LoadBinary(fullfile(basepath,[session.general.name '.lfp']),'nChannels',session.extracellular.nChannels,'channels',ch_theta,'precision','int16','frequency',srLfp)); % ,'start',start,'duration',duration
    Fpass = [4,11];
