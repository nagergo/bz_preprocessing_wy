function [ripples] = bz_FindRipples_wy(basepath, channel,varargin)
...
%                   of the stdev; must be integer values
...
%% TODO
% test if including noise channel could help 
%% Default values
p = inputParser;
addParameter(p,'thresholds',[2 2.5],@isnumeric)
addParameter(p,'inter_durations',10,@isnumeric)
...
addParameter(p,'minDuration',10,@isnumeric);
addParameter(p,'maxDuration',1000,@isnumeric);
addParameter(p,'plotType',2,@isnumeric);
addParameter(p,'save_evt',1,@isnumeric)
%%
...
minInterRippleInterval = p.Results.inter_durations;
maxRippleDuration = p.Results.maxDuration;
...
%% get lfp data
lfp = bz_GetLFP(channel,'basepath',basepath,'basename',basename);%==> bz_GetLFP use 0 indexing!!!
...
window = ones(windowLength,1)/windowLength; % window for filter0
keep = [];
if ~isempty(restrict)
...
[normalizedSquaredSignal,sd] = unity(Filter0(window,sum(squaredSignal,2)),sd,keep); % filter 0 is essentially a moving average filter

% Detect ripple periods by thresholding normalized squared signal
thresholded = normalizedSquaredSignal > lowThresholdFactor; % result will be logical
start = find(diff(thresholded)>0); % eg 000000011111100000 ==> transition from 0 to 1 will be start
stop = find(diff(thresholded)<0); %eg 000000011111100000 ==> transition from 1 to 0 will be end
...
if ~isempty(noise)
    if length(noise) == 1 % you gave a channel number
       noiselfp = bz_GetLFP(noise,'basepath',basepath,'basename',basename);%currently cannot take path inputs
...
if EMGThresh
    ...
    EMGfilename = fullfile(basepath,[sessionInfo.session.name '.EMGFromLFP.LFP.mat']);
...
num_ripple = length(rips);
[ripples.eventIDlabels{1:num_ripple,1}] = deal('rippleHSE');
ripples.eventID = 1:num_ripple;
...
detectorinfo.detectionparms.passband =  passband;
...
%%
n = length(ripples.timestamps);
d1 = cat(1,ripples.timestamps(:,1)',ripples.peaks',ripples.timestamps(:,2)');%DS1triad(:,1:3)';
events1.time = d1(:);
for i = 1:3:3*n
    events1.description{i,1} = ['RIP' ' start'];
    events1.description{i+1,1} = ['RIP' ' peak'];
    events1.description{i+2,1} = ['RIP' ' stop'];
end

if save_evt
    cd(save_folder)
    SaveEvents([basename '_RIP.RIP.evt'],events1);
end
%%
