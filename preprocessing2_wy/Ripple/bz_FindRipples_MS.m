function [ripples] = bz_FindRipples_MS(varargin)
...
addParameter(p,'durations',[20 1000],@isnumeric)
...
if isstr(varargin{1})  % if first arg is basepath
    ...
    lfp = bz_GetLFP(p.Results.channel+1,'basepath',p.Results.basepath,'basename',basename);%winnie added +1 ==> convert from 0 to 1 indexing ==> bz_GetLFP use 1 indexing!!!
...
%normalizedSquaredSignal = normalize(squaredSignal,'scale');
% Detect ripple periods by thresholding normalized squared signal

figure;subplot(3,1,1); plot(signal);
subplot(3,1,2); plot(squaredSignal);
subplot(3,1,3); plot(normalizedSquaredSignal);
...
if isfield(p.Results,'channel')
    detectorinfo.detectionchannel.zero = p.Results.channel;
    detectorinfo.detectionchannel.one = p.Results.channel+1;
end
if ~isempty(noise)
...
%% save .evt file for opening in neuroscope for inspection later
n = length(ripples.timestamps);
d1 = cat(1,ripples.timestamps(:,1)',ripples.peaks',ripples.timestamps(:,2)');
events1.time = d1(:);
for i = 1:3:3*n
