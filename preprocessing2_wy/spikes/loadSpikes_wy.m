function spikes = loadSpikes_wy(basepath,varargin)
...
% currrent version, Winnie Yang, Dec 2021, updated to the current version
% that ONLY load session instead of sessionInfo!!!!! This is generally true
% for Winnie's code
...
%%
%clusteringPath = fullfile(basepath,clusteringPath);
%%
...
if forceReload
    ...
    switch lower(clusteringFormat)
        ...
        case 'klustaViewa'
            ...
            shank_nb = 1;
            for shank = 1:shanks
                spike_times = double(hdf5read([clusteringPath, basename, '.kwik'], ['/channel_groups/' num2str(shank-1) '/spikes/time_samples']));
                recording_nb = double(hdf5read([clusteringPath, basename, '.kwik'], ['/channel_groups/' num2str(shank-1) '/spikes/recording']));
                cluster_index = double(hdf5read([clusteringPath, basename, '.kwik'], ['/channel_groups/' num2str(shank-1) '/spikes/clusters/main']));
                waveforms = double(hdf5read([clusteringPath, basename, '.kwx'], ['/channel_groups/' num2str(shank-1) '/waveforms_filtered']));
                clusters = unique(cluster_index);
                for i = 1:length(clusters(:))
                    cluster_type = double(hdf5read([clusteringPath, basename, '.kwik'], ['/channel_groups/' num2str(shank-1) '/clusters/main/' num2str(clusters(i)),'/'],'cluster_group'));
                    if cluster_type == 2
    ...
    % get brain region for each cell
     if ~isfield(spikes,'region') & isfield(session,'region')
        if ~isfield(spikes,'numcells')
            spikes.numcells = length(spikes.UID);
        end
        if isfield(spikes,'maxWaveformCh')
            for cc = 1:spikes.numcells
                spikes.region{cc} = session.region{spikes.maxWaveformCh(cc)==session.channels};
            end
        end
     end
...
if ~isempty(session)
    ...
    if ~isempty(session.channelTags.Bad)
